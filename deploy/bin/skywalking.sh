...
# add skywalking configs --------------------
SW_AGENT_SERS=10.60.0.46:11800
SERVICE_NAME=canal
JMX_REMOTE_PORT=1099

JAVA_JMX_OPTS=" -Dcom.sun.management.jmxremote.port=$JMX_REMOTE_PORT
...
SERVICE_HOME=`pwd`
cd bin
SW_AGENT_JAR=$SERVICE_HOME/agent/skywalking-agent.jar
JAVA_SW_AGENT=" -javaagent:$SW_AGENT_JAR -Dskywalking.agent.service_name=$SERVICE_NAME "
JAVA_SW_AGENT=$JAVA_SW_AGENT" -Dskywalking.collector.backend_service=$SW_AGENT_SERS "

$JAVA $JAVA_SW_AGENT $JAVA_DEBUG_OPT $CANAL_OPTS -classpath .:$CLASSPATH com.alibaba.otter.canal.deployer.CanalLauncher 1>>$base/logs/canal/canal.log 2>&1 &
# add skywalking confgis over -----------------
